...
A macOS desktop widget that shows the remaining quota of your local **Codex**, **Claude Code**, **Gemini**, and **ZCode** as red / yellow / green LEDs — so you can tell your usage at a glance without opening a terminal. At most two services are shown at once, and each desktop widget can be configured independently — so you can place several widgets, each showing different services.
...
- ⚙️ **Standard menu & settings** — the menu-bar menu has "About Quotient" (standard About panel: version, author Zhi Luo, built with the help of Claude Code) and "Settings…". Options:
  - **Show services** (max two): two provider pickers let you freely combine Codex / Claude / Gemini / ZCode; the second column can also be hidden. This is the floating panel's setting and also the default for new widgets;
  - **Language**: match system (default) / 简体中文 / English.
- 🧩 **Per-widget configuration** — each desktop widget has its own provider selection via the native "Edit Widget" pickers (long-press / right-click a widget → Edit Widget → First Provider / Second Provider). Leave First Provider on "Default (app setting)" to follow the panel, or pick a specific service / pair. Place multiple widgets, each showing different content.
- 🔄 **Auto refresh** — Codex and ZCode are read from local files every minute; Claude / Gemini are fetched every 5 minutes. It refreshes again right after each reset time, and windows past their reset show as 100%. When the Claude / Gemini access token expires, it is silently refreshed via the local refresh token (no re-login). When offline, the last result is kept and tagged with its "as of" time.
- 🔐 **Privacy-friendly**:
  ...
  - **Gemini** quota reuses Gemini CLI's login (`~/.gemini/oauth_creds.json`) and calls Code Assist's `retrieveUserQuota` endpoint (the same source as Gemini CLI's `/status`), showing the first two model families by priority (usually Pro / Flash).
  - **ZCode** quota comes from the local `billing/balance` snapshots written in `~/.zcode/v2/logs` or `~/.zcode/cli/log`, showing the remaining token quota per entitlement/model such as GLM-5.2 and GLM-5-Turbo.
  - Tokens are only used for the services that require live quota requests (Claude / Gemini), stay in memory during the request / refresh flow, and are written back only to their original stores after refresh — **never saved elsewhere, never displayed, never sent to any third party**. Codex and ZCode are parsed from local usage logs.

## Two display forms

1. **Liquid-glass floating panel** (the host app itself): pinnable, refreshes by the minute, controls appear on hover.
2. **System desktop widget** (WidgetKit extension): right-click the desktop → "Edit Widgets" → search "Quotient"; small and medium sizes, usable on the desktop and in Notification Center. Each instance is independently configurable (Edit Widget → First Provider / Second Provider). After upgrading from an older build, remove existing Quotient widgets and add them again so macOS registers the new two-provider configuration. In desktop monochrome rendering mode, bars switch to an opacity-contrast scheme.
...
```
Sources/
  Shared/   models, bilingual strings, Codex/Claude/Gemini/ZCode readers, shared views, snapshot
